...
(function(){
	...
	*/
	...
	QZAppExternal.report.returnCode = (function() {
		...
		var commurl = "https://c.isdspeed.qq.com/code.cgi",
...
(function(){
	if(window.mqq && !window.mqq.hackqqapi){
		...
		var funcMap = [['ui', 'popBack'], ['ui', 'showShareMenu'], [ 'ui','showTips'],['ui', 'openUrl'],
			...
			['media', 'showPicture', function (params, cbFn) {
				...
				window.__imageCallback =  function(e){
					...
					data.params.limit = data.params.limit || p.params.limit;
					...
					if(key && key === 'custom_image'){
						...
						if(e.data.current >= e.data.total - 1){
							...
							if(qzoneUpPhotoIndex==data.params.limit){ //全部照片集齐
